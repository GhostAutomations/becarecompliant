...
- [ ] E4k. LOCATION (Phil, amended after first pass): the Location is a dropdown, Office or Teams (migration 0050 adds branches.address, edited in Settings > Branches). Office prints the FULL branch address in both letters and the .ics; booking fails with a clear message if the branch has no address. Teams letters say the meeting is over Microsoft Teams and an invite will follow shortly. The card booked line shows the location. RETEST after deploy.
