...
- [ ] E4b. REWORKED (Phil's spec, 2026-07-12, migration 0051 publishes form v2): Job Title removed; Meeting Type is the first question and only offers the person's BOOKED stages (all four when nothing is booked); Manager Conducting, Date of Meeting and Purpose prefill from the earliest booking; the Attendance Record Review section prefills (current level from occasions/days, number of absences, dates list); NEW Meeting Minutes section. Everything stays editable before saving. Recording still attaches to the booking, sends no emails, and clears the booked line. Old evidence keeps form v1. TEST after deploy.
