...
- [ ] E4k. LOCATION (Phil, amended twice): the Location dropdown lists the company's NAMED offices, "{Company} Office" for the Team office then "{Branch} Branch Office" for each branch, plus Teams. Offices without an address show "(no address set)" and cannot be picked; the chosen office's FULL address (Settings > Branches) prints in both letters and the .ics. Teams letters say the meeting is over Microsoft Teams and an invite will follow shortly. The card booked line shows the location. RETEST after deploy.
- [ ] E4n. STAGE GATE (Phil): the stage dropdown only offers stages above the highest already held or booked (Stage 4 can repeat); the server enforces the same. The "no further action" reset ships with meeting outcomes (Additions).
- [ ] E4o. NO CTA for employees (Phil): the employee's invitation and cancellation emails have no "Open Be Care Compliant" button (they have no login); the conductor's copies keep it.
