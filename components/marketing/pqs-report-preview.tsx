  ...
  return (
    <div className="glass-card overflow-hidden p-0 text-left shadow-2xl shadow-black/40">
      <div className="flex items-center gap-2 border-b border-white/10 bg-white/[0.04] px-4 py-2.5">
        ...
        <span className="h-2.5 w-2.5 rounded-full bg-white/20" />
        <span className="h-2.5 w-2.5 rounded-full bg-white/20" />
        <span className="ml-2 text-xs text-white/55">PQS report, your service</span>
      </div>
      ...
      <div className="p-5">
        <div className="flex flex-wrap items-end justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-white/55">Provider Quality System return, last 6 months</p>
          </div>
          <span className="text-xs font-semibold text-gold-300">Export PDF or CSV</span>
        </div>
              ...
              {ROWS.map((r) => (
                <tr key={r.name}>
                  <td className="px-1 py-1.5">
                    ...
                    <span className="ml-2 text-[11px] text-white/55">{r.register}</span>
                  </td>
