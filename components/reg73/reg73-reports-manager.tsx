...
export default function Reg73ReportsManager({
  reports,
  canDelete,
}: {
  ...
  const [state, action, pending] = useActionState(deleteReg73Visits, IDLE_STATE);
  ...
  if (reports.length === 0) {
    return <div className="glass-card p-6 text-sm text-white/60">No Regulation 73 reports yet.</div>;
  }
          ...
          <tbody>
            {reports.map((r) => (
              <tr key={r.id} className="border-b border-white/5">
                ...
                <td className="px-3 py-2">
                  <Link href={`/reports/reg73/${r.id}`} className="font-medium text-white hover:text-gold-300">
                    {r.reference ?? `Visit ${fmtDate(r.start_date)}`}
                  </Link>
                </td>
