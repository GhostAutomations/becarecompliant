...
| # | Test | Result |
|---|------|--------|
| S1 | Bevan, ZZ TEST DBS Warn, Manage record: "Subject access export" shows for Bev Admin; press it; a ZIP comes back with README, summary.pdf, evidence.pdf, CSVs and files/updates with the Update attachments | PASS 24/09 (live, dpl_9Xhwa9mf: 15 KB, 14 files in 4 s, CSVs read correctly incl. earlier wording of an edited update; Phil opened summary.pdf: pass) |
| S2 | History on the record shows "Made a subject access export" | PASS 24/09 (audit "Made a subject access export for ZZ TEST DBS Warn (14 files)", sar_exports rows written) |
| S3 | The route refuses anybody who is not a Company Admin | By code: the route refuses any role but company_admin and support mode before reading anything; no second login run |
| S4 | From Phil's downloaded ZIP (24/09): summary.pdf header said "Compliance report", statuses printed raw ("active", "due"), the Leaving table headings ran into each other, and the Evidence line said "0 completed forms ... Original uploads are in files/evidence" with none there. Fixed: header "Subject access export", status labels, Leaving printed as labelled values, plain "No completed forms on this record." To see on the next export. | PASS 24/09 (Phil's re-export: heading, Active/Due, no forms line; ZZ TEST Leaver A: 4 leavings as labelled answers. Tidied after: reason and competitor not printed twice, "1 completed form ... in full") |
