...
import { createPublicLink, setPublicLinkEnabled } from "@/lib/public-forms/actions";
...
export default async function PublicFormsSettingsPage() {
  ...
  const [{ data: company }, links] = await Promise.all([
    supabase.from("companies").select("slug").eq("id", profile.company_id).maybeSingle(),
    listPublicFormLinks(profile.company_id),
  ]);

  const slug = (company?.slug as string | null) ?? "";
  const base = siteUrl();
      ...
      {!slug ? (
        <section className="glass-card p-5">
          <p className="text-sm text-white/70">
            Your company does not have a web address set yet, so public links cannot be
        ...
        <section className="space-y-4">
          {PUBLIC_FORM_DEFS.map((def) => {
            ...
            const url = `${base}${publicFormPath(slug, def.key)}`;
            return (
              <div key={def.key} className="glass-card p-5">
                ...
                {link ? (
                  <div className="mt-4 space-y-3">
                    <p className="break-all rounded-lg border border-white/10 bg-white/5 px-3 py-2 font-mono text-xs text-white/80">
                      {url}
                    </p>
