...
import { describeSubject, type EvidenceSubject } from "@/lib/evidence/subject-format";
...
export type EvidencePdfMeta = {
  /**
   * Who this evidence is ABOUT. Required, never optional: a Spot Check that does not
   * name the care worker is an anonymous document, and an anonymous document is worth
   * nothing to an inspector. Making it part of the type means no caller can render a
   * PDF without answering the question. See lib/evidence/subject.ts.
   */
...
const styles = StyleSheet.create({
  ...
  subjectBlock: { borderLeftWidth: 3, borderLeftColor: GOLD, paddingLeft: 8, paddingVertical: 2, marginTop: 8 },
  subjectLabel: { fontSize: 8, color: MUTED, textTransform: "uppercase" },
  subjectName: { fontSize: 13, fontWeight: 700, color: NAVY, marginTop: 1 },
  subjectMeta: { fontSize: 9, color: MUTED, marginTop: 1 },
...
export function EvidenceEntry({
  ...
  return (
    <>
      ...
      {/* The subject sits directly under the form name, ahead of everything else, so
          the first thing read off the page is whose record this is. */}
      <View style={styles.subjectBlock}>
        <Text style={styles.subjectLabel}>{meta.subject.label}</Text>
        <Text style={styles.subjectName}>{meta.subject.name}</Text>
        {meta.subject.reference || meta.subject.detail ? (
          <Text style={styles.subjectMeta}>
            {[meta.subject.detail, meta.subject.reference].filter(Boolean).join("  ·  ")}
          </Text>
        ) : null}
      </View>
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.brandBar}>
          ...
          <Text style={styles.evidenceTag}>
            {meta.subject.name}{"\n"}Evidence reference {meta.evidenceRef}
          </Text>
        </View>
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.footerText}>
            {describeSubject(meta.subject)}. Reference {meta.evidenceRef}. Immutable evidence
            generated by Be Care Compliant.
          </Text>
          <Text
